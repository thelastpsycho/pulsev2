...
- **react-markdown** for rendering AI chat assistant responses
...
- **`shortcuts-init.js`** - JavaScript keyboard shortcuts initialization (the one actually wired into app init — `shortcuts.ts` is not currently imported)
- **`cache.ts`** - Client-side caching utilities
- **`init.ts`** - App initialization utilities
- **`pulse-globals.js`** - Shared safe accessors (`getPulseUI`, `getPulseForm`, `getPulseOverlay`, `getPulseLayout`) for the `window.Pulse*` globals, with full fallback objects. Components render before `main.jsx` necessarily attaches these globals, so every consumer needs a fallback — this centralizes it instead of each component redefining its own subset inline. Not yet migrated everywhere; new components should use it rather than re-adding a local `getPulseUI()`.
...
- **`pages/`** - Page components (LoginPage, DashboardPage, IssuesPage, ChatPage, etc.)
- **`ui/`** - UI component library (UIPrimitives, UIForm, UIOverlay, GlobalSearch, InteractiveChart, MiniChart, etc.)
- **`reports/`** - Report generation components (`SimpleReportBuilder.jsx` is the one actually routed at `/reports/builder`; `ReportBuilder.tsx` exists but is not currently wired into any route)
...
window.PulseAPI.Chat        // callTool(name, args) - proxies to backend /api/ai/tools/{name}
```
...
### AI Chat Assistant (`/chat`, permission `ai.chat.view`)
- **`src/components/pages/ChatPage.jsx`** - natural-language queries over issues/rooms/guests/departments
- OpenAI (`gpt-4o-mini`) is called **directly from the browser** using `VITE_OPENAI_API_KEY` — tool calls are proxied through `window.PulseAPI.Chat.callTool` to the backend's `/api/ai/tools/{name}` (see `AiToolsService`/`AiChatController` in the Laravel backend). Note: a `VITE_*` env var is bundled into the client JS and is not actually secret — this key is publicly extractable from the built bundle.
- Tool-call loop: the model can chain up to `MAX_TOOL_ROUNDS` (5) tool calls in one turn before being forced to a final answer, rather than one fixed round-trip
- Available tools: `summary`, `room-search`, `guest-search`, `department-stats`, `urgent-issues`
- Assistant replies render as markdown via `react-markdown` (`MARKDOWN_COMPONENTS` in ChatPage.jsx gives compact overrides matching the chat bubble's type scale)
- Charts: when a tool result has 3+ comparable categorical values, the system prompt instructs the model to emit a fenced ` ```chart ` block (`{type: "bar"|"donut", title, data}`). `extractChart()` parses and strips it from the displayed/history text; `src/components/ui/MiniChart.jsx` renders it (bar/donut as inline SVG; a `line` type exists but currently falls back to bar since no tool returns time-series data yet)
